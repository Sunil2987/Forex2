...
useEffect(() => { fetchData(); timerRef.current = setInterval(() => { setCount((c) => { if (c <= 1) { if (refreshAllowed()) { fetchData(); return 300; } return c; } return c - 1; }); }, 1000); return () => clearInterval(timerRef.current); }, []);

const bulbs = (n) => '💡'.repeat(n);

return ( <div className="app"> <h1 className="header">Forex + BTC Volatility Monitor</h1>
